...
def carte_creations(regions_gdf,
    path_creation: str,
    ...
    ax.set_title("Taux moyen de création d'entreprises", fontsize=15, fontweight="bold")
    ax.set_axis_off()
    plt.show() 
...
def carte_educ1(
    ...
    col_region_geo: str = "LIBELLE_REGION",
    col_region_data: str = "region_nom",
    col_educ: str = "Taux de chômage par région",
    col_creation: str = "Création moyenne d'entreprise par an entre 2015 et 2025 par 1000 habitants",
):
    """
    Affiche deux cartes côte à côte :
    ...
    # Lecture des données
    educ = pd.read_csv(path_educ)
    creation = pd.read_csv(path_creation)

    creation = creation.rename(columns={"Region": col_region_data})

    # Merge géo + données
    gdf_educ = regions.merge(educ, on=col_region_data, how="left")
    gdf_creation = regions.merge(creation, on=col_region_data, how="left")
    ...
    gdf_educ.plot(
        column='Pourcentage_diplomes_superieur',
        ...
        legend_kwds={'label': "Taux de diplomes (%)", 'orientation': "horizontal", 'shrink': 0.7},
        ax=axes[0]            # C'est ici qu'on dit "mets-toi à gauche"
    )
    axes[0].set_title('Taux de diplomés Bac+5 et plus (2022)', fontsize=15, fontweight='bold')
    axes[0].set_axis_off()
    ...
    axes[1].set_title("Taux moyen de création d'entreprises par 1000 habitants", fontsize=15, fontweight='bold')
    axes[1].set_axis_off()
...
def carte_educ2(
    ...
    X = gdf_educ[
        ...
        "Pourcentage_Baccalauréat / Brevet professionnel ou équivalent",
        "Pourcentage_aucun_diplome_ou_certificat_d_etudes_primaires"
        ]
    ]
    X_clean = X.dropna()
    gdf_pca = gdf_educ.loc[X_clean.index].copy()
    X_scaled = StandardScaler().fit_transform(X_clean)
    pca = PCA(n_components=1)
    gdf_pca["indice_education_pca"] = pca.fit_transform(X_scaled)
    gdf_pca["indice_education_pca"] *= -1
    gdf_pca[["region_nom", "indice_education_pca"]].head()

    # 1. CRÉATION DE L'ESPACE (1 ligne, 2 colonnes)
    fig, axes = plt.subplots(1, 2, figsize=(20, 10))
        # --- CARTE 1 : CHÔMAGE (À GAUCHE -> axes[0]) ---
    gdf_pca.plot(
        column='indice_education_pca',
        cmap='OrRd',          # Rouge/Orange pour le chômage
        linewidth=0.8,
        edgecolor='0.8',
        legend=True,
        # 'shrink': 0.7 permet de réduire la taille de la barre de légende pour qu'elle ne dépasse pas
        legend_kwds={'label': "Indice ACP", 'orientation': "horizontal", 'shrink': 0.7},
        ax=axes[0]            # C'est ici qu'on dit "mets-toi à gauche"
    )
    axes[0].set_title('Indice ACP : Niveau de diplome (2022)', fontsize=15, fontweight='bold')
    axes[0].set_axis_off()
...
def build_regression3_csv(
    ...
    """
    Régression 3 :
    mêmes variables que la régression 2 + (taux de chômage)^2
    """
    ...
    # Nettoyage clés
    for df in (diplome, chomage, creation, population, salaires):
        if "region_nom" in df.columns:
            df["region_nom"] = (
                df["region_nom"].astype(str).str.replace("’", "'", regex=False).str.strip()
            )
